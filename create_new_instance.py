#!/usr/bin/env python3
"""
Create New MS AI Instance with Application Form
This script creates a new EC2 instance and deploys the enhanced application
"""
...
def create_new_instance():
    """Create a new EC2 instance with the enhanced MS AI application"""
    
    print("🚀 Creating New MS AI Instance with Application Form")
    print("=" * 60)
    ...
    # User data script for the new instance
...
'''
    ...
    try:
            ...
            TagSpecifications=[
                {
                    ...
                    'Tags': [
                        {'Key': 'Name', 'Value': 'msai-enhanced-app'},
        ...
        print("⏳ Waiting for application deployment to complete...")
        time.sleep(120)  # Wait 2 minutes for deployment
        
        # Test the new instance
        print("🧪 Testing new instance...")
        ...
        try:
            ...
            response = requests.get(f'http://{public_ip}:8000/application', timeout=10)
            if response.status_code == 200:
                print("✅ Application form accessible")
            else:
                print(f"⚠️  Application form returned status: {response.status_code}")
            
            print(f"\n🎉 New MS AI Instance Created Successfully!")
            print(f"🌐 Public IP: {public_ip}")
            print(f"🔗 Application: http://{public_ip}:8000")
            print(f"📝 Application Form: http://{public_ip}:8000/application")
            print(f"📚 API Documentation: http://{public_ip}:8000/docs")
            print(f"\n⏳ DNS propagation may take a few minutes...")
            print(f"🌐 Once propagated: http://msai.syzygyx.com")
            ...
            print(f"⚠️  Could not test instance immediately: {e}")
            print(f"🌐 Instance is running at: {public_ip}")
            print("⏳ Please wait a few minutes for deployment to complete")
            return {
                'instance_id': instance_id,
                'public_ip': public_ip,
                'status': 'deploying'
...
if __name__ == "__main__":
    ...
    if result:
        print(f"\n✅ Instance creation completed: {result['status']}")
    else:
