#!/usr/bin/env python3
"""
Setup Google Drive using service account authentication
"""
...
def get_service_account_token():
    ...
    success, stdout, stderr = run_gcloud_command("gcloud auth print-access-token --impersonate-service-account=msai-service-account@syzygyx-161202.iam.gserviceaccount.com")
    if success:
        return stdout.strip()
    else:
        print(f"❌ Error getting service account token: {stderr}")
        return None
...
def create_shared_drive(access_token):
    ...
    try:
        response = requests.post(url, headers=headers, json=data, params={"requestId": "msai-drive-002"})
...
def main():
    ...
    print("✅ Service account access token obtained")
